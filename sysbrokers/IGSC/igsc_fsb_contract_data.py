...
class IgscFsbContractData(brokerFuturesContractData):
    def __init__(self, log=logtoscreen("IgscFsbContractData")):
        super().__init__(log=log)
        self._igconnection = ConnectionIG()
        self._sierrachart = None
        self._instrument_data = IgscFsbInstrumentData(log=self.log)
        #self._bc_instrument_data = BarchartFuturesInstrumentData(log=self.log)

    def __repr__(self):
        return f"IGSC FSB per contract data: {self._igconnection}"
    ...
    @property
    def sierrachart(self):
        return self._sierrachart

    @property
    def ig_instrument_data(self) -> IgscFsbInstrumentData:
...
#        return self._bc_instrument_data
    ...
    def _get_contract_object_plus(
        ...
        if self._is_futures_spread_bet(contract_object):
            futures_contract_plus = (
                self.ig_instrument_data.get_igsc_fsb_instrument(
    ...
    def _get_actual_expiry_date_given_single_contract_plus(
        ...
        if self._is_futures_spread_bet(futures_contract_plus):
            # Note the IG Expiry date is in fact their own roll date
            # I dont think instrument.ExpiryOffset is ever applied to this date, so shoudn't be a problem
            # Will ensure the roll report days to expiry figure is relative
            ...
            raise RuntimeError('Contract is not futures spreadbet')
    ...
    def _is_futures_spread_bet(self, contract_object: futuresContract):
        ...
        return ("_fsb" in contract_object.instrument_code) or ("_JLSB" in contract_object.instrument_code)
